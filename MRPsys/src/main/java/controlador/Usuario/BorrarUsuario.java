 ...
 */
@WebServlet("/pages/BorrarUsuario")
public class BorrarUsuario extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (ApplicationService.getInstance().existsUsuario(username) == true) {
			ApplicationService.getInstance().borrarUsuario(username);
			request.getRequestDispatcher("/pages/main.jsp").forward(request, response);
		} else {
			mensaje = ApplicationService.getInstance().noExistePedidoException();
			request.getSession().setAttribute("mensaje", mensaje);
			request.getRequestDispatcher("/pages/LoadBorrarUsuario").forward(request, response);
		}
	}
}
